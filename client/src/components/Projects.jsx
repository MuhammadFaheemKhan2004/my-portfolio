    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                ...
                // Use default projects if API returns empty array
                if (response.data && response.data.length > 0) {
                    setProjects(response.data);
                } else {
                    console.log('No projects in database, using default projects');
                    setProjects(defaultProjects);
                }
                setLoading(false);
            } catch (err) {
                console.log('Using default projects (API error):', err.message);
                setProjects(defaultProjects);
                setLoading(false);
            }
        };
                    ...
                    {loading ? (
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {projects.map((project) => (
                                ...
                                >
                                    <div className="relative w-full max-w-sm mx-auto h-40 overflow-hidden bg-gradient-to-br from-accent/10 to-blue-600/10">
                                        <img
                                            src={project.image}
                                            alt={project.title}
                                    ...
                                    <div className="p-6">
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            {project.technologies.map((tech, i) => (
                                                <span
                                                    key={i}
                                                    className="px-2 py-1 bg-accent/10 text-accent text-xs rounded border border-accent/30 whitespace-nowrap"
                                                >
                                                    {tech}
                                                </span>
                                            ))}
